...
class ProductAdmin(ExportActionMixin, SummernoteModelAdmin, admin.ModelAdmin):
  ...
  def export_pricelist(self, request):
    ...
    for category in categories:
      ...
      for product in  Product.objects.filter(is_in_sales_price = True, category = category.id).order_by('title'):
        ...
        ws.append(['',product.title, str(product.price) + ' с НДС'])
        if product.image:
          try:
            ...
            pass
          ws['B' + str(counter)].font = Font(name='Calibri', size=14, color="09131f")
          ws['C' + str(counter)].font = Font(name='Calibri', size=14, bold=True, color="09131f")
          ws['B' + str(counter)].alignment = Alignment(horizontal='left', vertical='center')
          ws['C' + str(counter)].alignment = Alignment(horizontal='left', vertical='center')
      ws.append('','','')
    buffer = io.BytesIO()  
    wb.save(buffer)  
    buffer.seek(0)  
    # Формируем HttpResponse  
    response = HttpResponse(buffer, content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")  
    response["Content-Disposition"] = "attachment; filename=pricelist_Katran.xlsx"  
    return response 
  ...
  fields = (
      ...
      "is_features", "is_sale", "is_bestseller", "is_visible", "is_in_sales_price", "keywords", "description", "image",
